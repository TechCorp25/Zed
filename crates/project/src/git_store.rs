...
pub fn worktrees_directory_for_repo(
    ...
    let worktree_directory = shellexpand::tilde(worktree_directory_setting);
    let resolved = if path_style.is_absolute(&worktree_directory) {
        PathBuf::from(path_style.normalize(&worktree_directory))
    } else {
        path_style.join_path(repository_anchor_path, worktree_directory.as_ref())?
    };
    let resolved = if path_style
        .strip_prefix(&resolved, repository_anchor_path)
        .is_some()
    {
        resolved
    ...
    {
        anyhow::bail!("git.worktree_directory must not resolve to the repository directory");
    }
